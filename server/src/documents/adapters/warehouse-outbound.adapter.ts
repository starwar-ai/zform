/**
 * 出库单 Adapter (Warehouse Outbound)
 */
 ...
 * 出库单状态流转配置
 */
const OUTBOUND_ORDER_STATUS_CONFIG = {
  transitions: {
...
export const warehouseOutboundAdapter: DocumentTypeAdapter = {
  ...
  async beforeDelete(id: string, prismaClient: any) {
    ...
    if (['IN_PROGRESS', 'COMPLETED'].includes(order.orderStatus)) {
      throw new Error(`出库单 ${order.code} 状态为 ${order.orderStatus}，不允许删除`);
    }
  },
  ...
  actions: {
    ...
    async approve({ id, body, userId, prisma }) {
      ...
      if (order.orderStatus !== 'PENDING') {
        throw new Error(`出库单 ${order.code} 状态为 ${order.orderStatus}，无法审核`);
      }
      ...
      return {
        data: doc,
        message: `出库单${approved ? '审核通过' : '审核拒绝'}`,
      };
    },
    ...
    async createFromNotice({ body, userId, prisma }) {
      ...
      const lastOrder = await prisma.warehouseOrder.findFirst({
        where: { code: { startsWith: prefix }, type: 'OUTBOUND' },
        orderBy: { code: 'desc' },
      });
      let seq = 1;
      if (lastOrder) {
