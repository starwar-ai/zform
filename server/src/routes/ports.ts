...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const port = await portService.findById(ensureString(req.params.id));
    if (!port) {
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const port = await portService.update(ensureString(req.params.id), req.body, userId);
    res.json(port);
  } catch (error) {
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    await portService.delete(ensureString(req.params.id));
    res.status(204).send();
  } catch (error) {
