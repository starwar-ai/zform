...
import {
  getPaymentOverdueReportApi,
  type PaymentPlanItem,
  type PaymentOverdueReportSummary,
} from "@/apis/payment-workbench-api"

interface PaymentOverdueReportProps {
...
export function PaymentOverdueReport({ onOpenContract }: PaymentOverdueReportProps) {
  const [loading, setLoading] = useState(false)
  const [data, setData] = useState<PaymentPlanItem[]>([])
  const [summary, setSummary] = useState<PaymentOverdueReportSummary | null>(null)
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const result = await getPaymentOverdueReportApi({
        page,
        pageSize,
        ...filters,
      })
      ...
      console.error("获取付款超期报表失败:", error)
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = () => {
    ...
    const rows = data.map((item) => [
      ...
      item.purchaseContract?.supplierCode ?? "",
      item.purchaseContract?.supplierName ?? "",
      `第${item.periodIndex}期`,
      formatDate(item.expectedPaymentDate),
      item.overdueDays,
      item.periodPayable,
      item.periodPaid,
      item.periodPayable - item.periodPaid,
    ])
    ...
    link.download = `付款超期报表_${new Date().toISOString().split("T")[0]}.csv`
    link.click()
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFilters((prev) => ({
                      ...prev,
                      buyerCode: e.target.value || undefined,
                    }))
                  }
                />
              </div>
                ...
                <TableBody>
                  {data.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.purchaseContract?.supplierCode ?? "-"}</TableCell>
