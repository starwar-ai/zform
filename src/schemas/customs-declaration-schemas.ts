/**
 * Customs Declaration Schemas
 *
 * 报关单单据定义（基于后端 Prisma Schema）
 */
...
export const customsDeclarationSchema: DocumentSchema = {
  ...
  detailTables: [
    {
      ...
      fields: [
        ...
        { id: "customsDocument", label: "报关单据", type: "text" },
        { id: "pricingMethod", label: "计价方式", type: "text" },
        { id: "shippingLocation", label: "发货地点", type: "text" },

        // === 来源明细 ===
        { id: "sourceDetailId", label: "出运明细明细主键", type: "text" },
...
export const customsDeclarationChangeRule: ChangeRule = {
  typeId: "customs_declaration",
  watchFields: [
    "master.shippingDate",
  ...
  evaluate: (oldDoc, newDoc, downstreamDocs) => {
    ...
    if (oldDate !== newDate && downstreamDocs.length > 0) {
      for (const ds of downstreamDocs) {
        impacts.push({
          ...
          description: `报关单的出运日期变更，可能影响下游单据 ${ds.docNumber}`,
        })
      }
    }
    ...
    for (const newItem of newItems) {
      ...
      if (oldItem) {
        const oldQty = Number(oldItem.data.customsQuantity) || 0
        const newQty = Number(newItem.data.customsQuantity) || 0
        if (newQty < oldQty) {
          impacts.push({
            level: "critical",
            ...
            affectedField: "customsQuantity",
            description: `报关数量从 ${oldQty} 减少到 ${newQty}，可能影响已完成的报关`,
          })
        }
      }
    }
