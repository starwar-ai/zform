/**
 * Inspection Declaration Schemas
 *
 * 商检单单据定义（基于后端 Prisma Schema）
 */

import type { DocumentSchema, ChangeRule } from "@/core/types"

// ============================================================
// 商检单 (Inspection Declaration)
// ============================================================

export const inspectionDeclarationSchema: DocumentSchema = {
...
export const inspectionDeclarationChangeRule: ChangeRule = {
  typeId: "inspection_declaration",
  watchFields: [
    "master.shippingDate",
    ...
    if (oldDate !== newDate && downstreamDocs.length > 0) {
      for (const ds of downstreamDocs) {
        impacts.push({
          ...
          description: `商检单的出运日期变更，可能影响下游单据 ${ds.docNumber}`,
        })
      }
    }
      ...
      if (oldItem) {
        ...
        if (newQty < oldQty) {
          impacts.push({
            ...
            description: `报关数量从 ${oldQty} 减少到 ${newQty}，可能影响已关联的报关单据`,
          })
        }
      }
    }
