/**
 * Collection Reminder Store (收款提醒 Store)
 *
 * 管理收款提醒状态，通过轮询获取新提醒
 */
...
import {
  ...
  getUnreadCountApi,
  getReminderStatsApi,
  markReminderAsReadApi,
  markAllRemindersAsReadApi,
  type CollectionReminder,
  type ReminderStats,
} from "@/apis/collection-workbench-api"

interface CollectionReminderStoreState {
...
export const useCollectionReminderStore = create<CollectionReminderStoreState>()(
  immer((set, get) => ({
    reminders: [],
    stats: null,
    unreadCount: 0,
    ...
    fetchStats: async () => {
      try {
        const stats = await getReminderStatsApi()
        set((s) => {
          s.stats = stats
    ...
    markAsRead: async (id) => {
      try {
        await markReminderAsReadApi(id)
        set((s) => {
    ...
    markAllAsRead: async () => {
      try {
        await markAllRemindersAsReadApi()
        set((s) => {
